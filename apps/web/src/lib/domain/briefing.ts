...
export async function generateBriefing(
  ...
  if (
    ...
    throw new Error('CONFLICT: briefing is not available for this trip');
  }

  if (kind === 'pretrip_t24' && trip.status !== 'PLANNED') {
    throw new Error('CONFLICT: pre-trip briefing requires a planned trip');
  }
  ...
  const existing = await db.briefingGeneration.findUnique({
    where: { generationKey },
  });
  if (existing) return existing;

  const next = trip.segments.find(
    s =>
      s.departureUtc &&
      s.departureUtc.getTime() >= travelDate.getTime(),
  );
  ...
  const aiConfigured = Boolean(
    env.AIROUTER_API_KEY || env.GEMINI_API_KEY || env.OPENAI_API_KEY,
  );
  ...
  if (aiConfigured) {
    try {
      ...
      content = deterministicBriefing(dateKey, next);
    }
  } else {
    content = deterministicBriefing(dateKey, next);
  }
  ...
  return db.$transaction(async tx => {
    const generated = await tx.briefingGeneration.create({
      data: {
        tenantId,
        tripId,
        travelDate: new Date(`${dateKey}T00:00:00Z`),
        content,
        generatedAt: new Date(),
        generationKey,
        model,
        policyVersion: env.OPENAI_EXTRACTION_POLICY_VERSION,
      },
    });
    ...
    await recordEvent(tx, {
      tenantId,
      tripId,
      eventName: 'DAILY_BRIEFING_GENERATED',
      actorType: 'SYSTEM',
      payload: {
        briefingGenerationId: generated.briefingGenerationId,
        slot: kind,
        travelDate: dateKey,
      },
    });
...
function deterministicBriefing(
  dateKey: string,
  next: { departureLocation: string | null; arrivalLocation: string | null; bookingReference: string | null } | undefined,
): BriefingContent {
  return {
    TODAY: `Your trip wallet is ready for ${dateKey}.`,
    TOMORROW: 'Check the Wallet for the next confirmed segment.',
    READY_FOR_NEXT_STOP: next
      ? `${next.departureLocation ?? 'Departure'} → ${
          next.arrivalLocation ?? 'next stop'
        }${next.bookingReference ? ` · ${next.bookingReference}` : ''}`
      : 'No upcoming segment found in canonical trip data.',
    ONE_THING_YOU_DIDNT_KNOW:
      'This briefing uses canonical trip data; it does not monitor live disruptions.',
    SAFETY_BRIEF:
      'Keep original booking evidence accessible and verify supplier instructions directly when needed.',
  };
}

export async function viewBriefing(
  tripId: string,
  tenantId: string,
  actorId: string,
  kind: BriefingKind = 'morning',
  ...
  await recordEvent(db, {
    tenantId,
    tripId,
    eventName: 'DAILY_BRIEFING_VIEWED',
    actorType: 'USER',
    actorId,
    behavioralClass: 'BRIEFING',
    payload: {
